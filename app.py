...
GEMINI_MODEL = "gemini-1.5-pro" #gemini-1.5-flash gemini-2-flash
...
SQL_GENERATION_PROMPT = '''Você é um assistente de IA que gera consultas SQL com base em perguntas do usuário, esquema do banco de dados e substantivos únicos encontrados nas tabelas relevantes. Gere uma consulta SQL válida para responder à pergunta do usuário.
...
Resposta: SELECT estado, COUNT(*) as total FROM convenios GROUP BY estado

2. Qual é a média de cartões por convênio?
Resposta: SELECT c.nome, COUNT(ca.id) as total_cartoes FROM convenios c LEFT JOIN cartoes ca ON c.id = ca.convenio_id GROUP BY c.nome

3. Qual é a distribuição de status dos convênios?
Resposta: SELECT status, COUNT(*) as total FROM convenios GROUP BY status'''

SQL_VALIDATION_PROMPT = '''Você é um assistente de IA que valida e corrige consultas SQL. Sua tarefa é:
1. Verificar se a consulta SQL é válida.
2. Garantir que todos os nomes de tabelas e colunas estejam corretamente escritos e existam no esquema. Todos os nomes de tabelas e colunas devem estar entre crases.
3. Se houver algum problema, corrigi-los e fornecer a consulta SQL corrigida.
4. Se nenhum problema for encontrado, retornar a consulta original.

Responda no formato JSON com a seguinte estrutura. Apenas responda com o JSON:
{
    "valid": boolean,
...
Importante:
- Use FORMAT_DATETIME('%Y-%m', DATETIME(campo_data)) para extrair ano e mês de um campo datetime
...
{{
    ...
    "corrected_query": string
        ...
        try:
            ...
            # Remove marcadores de código e espaços em branco extras
            cleaned_response = response
